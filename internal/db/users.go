...
func (s *Svc) GetUserByUserID(userID int64) (*User, error) {
	var user *User
	rows, err := s.Db.Query("SELECT * FROM users where userID = ?", int(userID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Svc) UpdateUserWithIDAndUsernameByLicenseKey(userID int64, userName, licenseKey string) error {
...
func (s *Svc) UpdateUserWithRulesAcceptedByUserID(userID int64, rulesAccepted bool) error {
...
func (s *Svc) UpdateUserBinanceKeysAndTimestamp(userID int64, apiKey, apiSecret string) error {
...
func (s *Svc) UpdateUserNotFilledOrderIDs(userID int64, orderIDs string) error {
...
func (s *Svc) UpdateUserTimestamp(userID int64, unix int64) error {
